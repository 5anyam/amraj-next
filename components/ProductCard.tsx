  ...
  return (
    <div className="shadow rounded-xl bg-white border border-blue-50 dark:bg-[#232144] dark:border-blue-800 p-3 transition-transform hover:scale-105 flex flex-col">
      <Link href={productUrl}>
        ...
        <div className="font-bold text-xl text-blue-600 dark:text-blue-200 mt-1">₹{product.price}</div>
        <div className="text-gray-500 dark:text-gray-300 text-xs mt-1 line-clamp-2">{product.short_description?.replace(/<[^>]+>/g, "")}</div>
      </Link>
    </div>
  );
}
