    ...
    <div className="mb-6 flex flex-col gap-2">
      <div className="flex gap-5">
        {OFFERS.map((offer) => (
          <button
            key={offer.label}
            className={`flex-1 relative px-4 py-5 border rounded-xl shadow-sm transition-all text-left
            ...
            type="button"
          >
            <span className="absolute -top-3 left-3 bg-[#168b3f] text-white text-xs font-bold px-3 py-1 rounded-lg shadow">
              Save ₹{getSaveAmount(offer)}
            </span>
            <div className="font-bold text-lg text-[#168b3f]">{offer.duration}</div>
            <div className="text-sm mt-1 text-gray-700">Pack Of {offer.qty * 4} Tubes
              <span className="ml-1 text-gray-400">({offer.qty * 60} Tablets)</span>
            </div>
            <div className="mt-2 text-2xl font-bold text-[#168b3f]">
            ...
            {selected.label === offer.label && (
              <div className="absolute bottom-0 right-0 m-2 px-3 py-1 text-xs rounded bg-green-200 text-green-800 font-semibold">
                Best Results
              </div>
            )}
          </button>
